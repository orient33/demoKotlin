...
/**
 * support库中的SwipeRefreshLayout. 改成横向.
 * 参考 https://www.jianshu.com/p/c5f1867e7df1
 */
public class SwipeRefreshLayoutHor extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
     ...
     * @param context
     */
    public SwipeRefreshLayoutHor(@NonNull Context context) {
        this(context, null);
    }
     ...
     * @param attrs
     */
    public SwipeRefreshLayoutHor(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void setRefreshing(boolean refreshing, final boolean notify) {
        if (mRefreshing != refreshing) {
            ...
            ensureTarget();
            mRefreshing = refreshing;
            finishSpinner(0f);//reset to idle.!
//            if (mRefreshing) {
...
//            }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final int childLeft = getPaddingLeft() + Math.max(0, mCurrentTargetOffsetLeft);
...
//                (width / 2 + circleWidth / 2), mCurrentTargetOffsetLeft + circleHeight);
    ...
    public void onNestedPreScroll(@NonNull View target, int dx, int dy, @NonNull int[] consumed) {
        ...
        TopUtilKt.log("onNestedPreScroll. " + mTotalUnconsumed + ", dx=" + dx, "ddf");
        ...
        TopUtilKt.log("onNestedPreScroll. consumed = " + parentConsumed[0], "ddf");
    ...
    public void onNestedScroll(@NonNull final View target, final int dxConsumed, final int dyConsumed,
        ...
        if (dx < 0 && !canChildScrollLeft()) {
            ...
            TopUtilKt.log("onNestedScroll. " + mTotalUnconsumed, "ddf");
            moveSpinner(mTotalUnconsumed);
        }
    }
    ...
    private void moveSpinner(float overscrollTop) {
        ...
        float slingshotDist = mCustomSlingshotDistance > 0 ? mCustomSlingshotDistance :
                (mUsingCustomStart ? mSpinnerOffsetEnd - mOriginalOffsetLeft : mSpinnerOffsetEnd);
        ...
        TopUtilKt.log("moveSpinner: offsetLeft=" + mCurrentTargetOffsetLeft +
                        ", over scroll=" + (int) overscrollTop + ", targetX= " + targetX +
                        ", , totalDistance= " + (int) mTotalDragDistance
                , "ddf");
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop > 0) {
                        TopUtilKt.log("touch, move. dx= " + (x - mInitialMotionX), "ddf");
    ...
    private void startDragging(float x) {
        final float diff = x - mInitialDownX;
